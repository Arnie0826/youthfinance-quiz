...
const allData = {
  ...
  advanced: [
    ...
    {
      q: "What is a bond?",      options: ["A loan to the government or company", "Stock", "Interest rate", "Cryptocurrency"],      answer: 0,      feedback: "Bonds are like lending money to a government or business."    },    {      q: "What is risk tolerance?",      options: ["How much loss you can handle", "Interest rate", "Investment type", "Spending ability"],      answer: 0,      feedback: "Risk tolerance is how much financial loss you can handle emotionally."    },    {      q: "What is diversification in finance?",      options: ["Spreading investments to lower risk", "Investing in only crypto", "Buying one company’s stock", "Spending it all"],      answer: 0,      feedback: "Diversification means not putting all your money in one investment."    },    {      q: "What’s the difference between a debit and a credit card?",      options: ["Debit uses your money, credit borrows from bank", "Both are same", "Credit is cheaper", "Debit pays interest"],      answer: 0,      feedback: "Debit uses your funds; credit borrows from the bank."    },    {      q: "Why do people invest in mutual funds?",      options: ["To reduce risk through variety", "To avoid taxes", "To shop online", "To buy gold"],      answer: 0,      feedback: "Mutual funds spread risk by investing in many things at once."    },    {      q: "What is a financial goal?",      options: ["Something you want to achieve with your money", "A gift card", "A product", "A discount"],      answer: 0,      feedback: "A financial goal is a plan for your money, like saving for a car."    },    {      q: "What is net income?",      options: ["Income after taxes and expenses", "Total income", "Cash savings", "Bonus"],      answer: 0,      feedback: "Net income is what you take home after taxes and bills."    }  ]};
...
(() => {
  ...
  const quiz = allData[level];
  if (!quiz) {
    console.error('No quiz data for level', level);
    return;
  }
  ...
  // Cache DOM
  const progText  = document.getElementById('quizProgress');
  const progBar   = document.getElementById('quizProgressBar');
  const quesEl    = document.getElementById('quizQuestion');
  const choicesEl = document.getElementById('quizChoices');
  const nextBtn   = document.getElementById('nextBtn');

  if (!progText || !progBar || !quesEl || !choicesEl || !nextBtn) {
  ...
  function renderQuestion() {
    ...
    choicesEl.innerHTML = '';
    progText.textContent = `Question ${current + 1} of ${quiz.length}`;
    progBar.style.width  = `${(current / quiz.length) * 100}%`;

    const { q, options } = quiz[current];
    quesEl.textContent   = q;
    ...
    nextBtn.textContent = current < quiz.length - 1 ? 'Next' : 'Submit';
  }

  nextBtn.addEventListener('click', () => {
    const sel = document.querySelector('.quiz-choice.selected');
    if (!sel) return;
    const idx = parseInt(sel.dataset.index, 10);
    const corr = quiz[current].answer;

    if (idx === corr) score++;
    else wrongAnswers.push({
      q:        quiz[current].q,
      correct:  quiz[current].options[corr],
      feedback: quiz[current].feedback
    });
    ...
    if (current < quiz.length) {
      ...
      window.location.href = 'quiz-ai-loading.html';
    }
  });
